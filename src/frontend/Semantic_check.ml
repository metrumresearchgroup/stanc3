  ...
  let rec aux k ut xs =
    ...
    | _, next :: rest -> (
      ...
      | _ -> (
        ...
        | _ -> Semantic_error.not_indexable loc ut |> Validate.error ) ) in
...
and semantic_check_funapp ~is_cond_dist id es cf emeta =
  let name_check =
    ...
    else semantic_check_fn_conditioning in
  Validate.(
    es
    |> List.map ~f:(semantic_check_expression cf)
    |> sequence
...
let semantic_check_transformation cf = function
  ...
  | LowerUpper (e1, e2) ->
      ...
      and ue2 =
        semantic_check_expression_of_int_or_real_type cf e2 "Upper bound" in
  ...
  | OffsetMultiplier (e1, e2) ->
      ...
      and ue2 =
        semantic_check_expression_of_int_or_real_type cf e2 "Multiplier" in
...
let semantic_check_assignment_operator ~loc assop lhs rhs =
  Validate.(
    ...
    match assop with
    | Assign | ArrowAssign ->
        if check_of_same_type_mod_array_conv "" lhs.emeta.type_ rhs.emeta.type_
        then
...
let semantic_check_assignment ~loc ~cf assign_id assign_indices assign_op
  ...
  and block =
    ...
    |> Option.value
         ~default:
           ( if is_stan_math_function_name assign_id.name then
             Validate.ok MathLibrary
           else
             Validate.error
             @@ Semantic_error.ident_not_in_scope loc assign_id.name ) in
  Validate.(
...
and semantic_check_loop_body ~cf loop_var loop_var_ty loop_body =
  ...
  let us =
    ...
    |> Validate.apply_const is_fresh_var in
...
and semantic_check_block ~loc ~cf stmts =
  ...
  *)
  let validated_stmts =
    List.map ~f:(semantic_check_statement cf) stmts |> Validate.sequence in
...
and semantic_check_size_decl ~loc is_global sized_ty =
  let not_ptq e =
    match e.emeta.ad_level with AutoDiffable -> false | _ -> true in
...
and semantic_check_var_decl_initial_value ~loc ~cf id init_val_opt =
  ...
  |> Option.value_map ~default:(Validate.ok None) ~f:(fun e ->
         ...
         |> Validate.map ~f:(fun ts ->
                ...
                | _ ->
                    let msg =
                      "semantic_check_var_decl: `Assignment` expected." in
                    fatal_error ~msg ()))
...
and semantic_check_fundef_dist_rt ~loc id return_ty =
  Validate.(
    let is_dist =
      List.exists
        ~f:(fun x -> String.is_suffix id.name ~suffix:x)
        ["_log"; "_lpdf"; "_lpmf"; "_lcdf"; "_lccdf"] in
    if is_dist then
  ...
  Validate.(
    ...
    *)
    ...
    and context =
      ...
      ; in_returning_fun_def= urt <> Void } in
...
let semantic_check_program
  ...
  let ufb =
    Validate.(
      ...
      >>= fun xs ->
      semantic_check_functions_have_defn xs |> map ~f:(fun _ -> xs)) in
  ...
  let mk_typed_prog ufb udb utdb upb utpb umb ugb : Ast.typed_program =
    ...
    ; modelblock= umb
    ; generatedquantitiesblock= ugb } in
