...
let use_file filename =
  ...
  if not !pretty_print_program then (
    ...
    if !dump_mir then
      Sexp.pp_hum Format.std_formatter [%sexp (mir: (Middle.Expr.Typed.meta Middle.Expr.t, (Middle.Expr.Typed.meta, Middle.Stmt.Typed.meta) Middle.Stmt.t ) Middle.Program.t ) ] ;
