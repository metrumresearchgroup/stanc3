...
module type Frontend = sig
  ...
  val mir_of_file :
       opts:frontend_opts
    -> file:string
    -> (Mir.Program.Typed.t, frontend_error) result

  val mir_of_string :
    opts:frontend_opts -> str:string -> (Mir.Program.Typed.t, frontend_error) result
end
...
module type Backend = sig
  ...
  val mir_to_repr : opts:backend_opts -> Mir.Program.Typed.t -> repr
  val mir_to_string : opts:backend_opts -> Mir.Program.Typed.t-> string
end
...
module type Optimization = sig
  ...
  val optimize : opts:optimization_opts -> Mir.Program.Typed.t -> Mir.Program.Typed.t
end
