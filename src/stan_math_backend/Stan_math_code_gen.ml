...
let maybe_templated_arg_types args  =
  let is_autodiff (adtype, _, _) =
    match adtype with UnsizedType.AutoDiffable -> true | _ -> false
  in
  List.filter ~f:is_autodiff args |> List.mapi ~f:(fun i _ -> sprintf "T%d__" i)

let%expect_test "arg types templated correctly" =
  [(AutoDiffable, "xreal", UnsizedType.UReal); (DataOnly, "yint", UInt)]
...
let to_indexed assignee idcs =
  { Expr.expr= Indexed ({expr= Var assignee; emeta= internal_meta}, idcs)
  ; emeta= internal_meta }

(** [pp_located_error_b] automatically adds a Block wrapper *)
let pp_located_error_b ppf (body_stmts, err_msg) =
  pp_located_error ppf
    (pp_statement, {Stmt.stmt= Block body_stmts; smeta= no_span}, err_msg)
...
let pp_fun_def ppf = function
  | {FunDef.fdrt; fdname; fdargs; fdbody; _} -> (
      ...
      in
      pp_sig ppf fdname ;
      match Stmt.pattern fdbody with
      | Stmt.Pattern.Skip -> pf ppf ";@ "
      | _ ->
...
let%expect_test "dims" =
  let v s = {Expr.expr= Var s; emeta= internal_meta} in
...
let pp_ctor ppf (p : Program.Typed.t) =
  let params =
  ...
  let get_param_st = function
    | _, (st, Program.Parameters) -> (
      match get_dims st with
      | [] -> Some [{Expr.expr= Lit (Int, "1"); emeta= internal_meta}]
      | ls -> Some ls )
    | _ -> None
  in
  let rec pp_statement_zeroing ppf s =
    ...
    | Stmt.Pattern.Decl {decl_id; decl_type; _} -> (
      ...
      | Unsized _ -> () )
    | Stmt.Pattern.Block ls | SList ls -> (list ~sep:cut pp_statement_zeroing) ppf ls
  ...
  pp_block ppf
    ( (fun ppf p ->
        ...
        pp_function__ ppf (Program.name p, Program.name p) ;
        pp_located_error ppf
          ( pp_statement_zeroing
          , {Stmt.stmt= Block p.prepare_data; smeta= no_span}
          , "inside ctor" ) ;
        cut ppf () ;
...
let pp_model_private ppf p =
  let decl = function
    ...
    | _ -> None
  in
  let data_decls = List.filter_map ~f:decl (Program.prepare_data p) in
...
let pp_get_param_names ppf p =
  ...
  pp_method ppf "void" "get_param_names" ["std::vector<std::string>& names__"]
    [] (fun ppf ->
      ...
      (list ~sep:cut add_param) ppf (List.map ~f:fst (Program.output_vars p)) )
...
let pp_get_dims ppf p =
  ...
  let pp_output_var ppf =
    (list ~sep:pp_dim_sep (list ~sep:cut pp_dim))
      ppf
      List.(map ~f:get_dims (map ~f:(fun (_, (st, _)) -> st) (Program.output_vars p)))
  in
...
let pp_write_array ppf p =
  ...
  let intro =
    ...
    ; strf "%a" pp_function__ (Program.name p, "write_array")
...
let separated_output_vars p =
  List.partition3_map
    ~f:(function
      | id, (st, Program.Parameters) -> `Fst (id, st)
...
let pp_transform_inits ppf p =
  ...
  in
  pp_method_b ppf "void" "transform_inits" params intro (Program.transform_inits p)
...
let pp_log_prob ppf p =
  ...
  let intro =
    ...
    ; strf "%a" pp_function__ (Program.name p, "log_prob")
    ; "stan::io::reader<local_scalar_t__> in__(params_r__, params_i__);" ]
  in
  let outro = ["lp_accum__.add(lp__);"; "return lp_accum__.sum();"] in
  let rec add_jacobians {Stmt.stmt; smeta} =
    match stmt with
    | Stmt.Pattern.Assignment (lhs, {Expr.expr= FunApp (CompilerInternal, f, args); emeta})
      when internal_fn_of_string f = Some FnConstrain ->
        let var n = {Expr.expr= Var n; emeta= internal_meta} in
        let assign rhs = {Stmt.stmt= Assignment (lhs, rhs); smeta} in
        { Stmt.stmt=
            IfElse
              ( var "jacobian__"
              , assign
        ...
        ; smeta }
    | _ -> {stmt= Stmt.Pattern.map Fn.id add_jacobians stmt; smeta}
  in
...
let pp_model ppf (p : Program.Typed.t) =
...
let%expect_test "udf" =
  let with_no_loc stmt = {Stmt.stmt; smeta= no_span} in
  let w e = {Expr.expr= e; emeta= internal_meta} in
  { fdrt= None
  ; fdname= "sars"
