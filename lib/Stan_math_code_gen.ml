...
let ifelse condid yes no sloc =
  ...
  with_loc
    (IfElse
       ( {internal_expr with texpr= Var condid}
       , with_loc (Block [yes])
       , Some (with_loc (Block [no])) ))
...
let rec size_of tvtype tvtrans =
  match tvtype with
  | Ast.SInt | Ast.SReal -> [{internal_expr with texpr= Lit (Int, "0")}]
  | Ast.SVector d | Ast.SRowVector d -> [d]
  | Ast.SMatrix (d1, d2) ->
...
let trans_read_and_constrain tvd =
  ...
  let fill_param cname id args =
    let read =
      { texpr=
          FunApp
            ( "in__." ^ cname ^ "_constrain"
            , size_of tvd.tvtype tvd.tvtrans @ args )
  ...
  let helper cname args =
    let forl =
      if is_scalar_constraint tvd.tvtrans then Ast_to_Mir.for_scalar
      else Ast_to_Mir.for_eigen
    in
    let fill_jac id =
      ifelse "jacobian__"
        (fill_param cname id (args @ [{internal_expr with texpr= Var "lp__"}]))
        (fill_param cname id args) tvd.tvloc
    in
    [forl ut fill_jac tvdvar]
  in
  ...
  | Offset o ->
      helper "offset_multiplier" [o; {internal_expr with texpr= Lit (Int, "1")}]
  | Multiplier m ->
      helper "offset_multiplier" [{internal_expr with texpr= Lit (Int, "0")}; m]
