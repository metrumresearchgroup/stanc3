...
(** [mkfor] returns a MIR For statement that iterates over the given expression
    [iteratee]. *)
let mkfor ut bodyfn iteratee =
  let idx s =
    match ut with
    (*  | Ast.UMatrix -> MatrixSingle (Var s)
*)
    | Ast.UVector | URowVector | UMatrix | UArray _ ->
        let expr_typed = Ast.Variable {name= s; id_loc= iteratee.texpr_loc} in
        Ast.Single
          { Ast.expr_typed_loc= iteratee.texpr_loc
          ; expr_typed
  ...
  let stmt =
    For
      ...
      ; body=
          ...
          ; sloc= iteratee.texpr_loc } }
  in
  reset () ;
  {stmt; sloc= iteratee.texpr_loc}

(** [for_scalar unsizedtype...] generates a For statement that loops
    over the scalars in the underlying [unsizedtype] *)
let rec for_scalar ut bodyfn var =
  match ut with
  | Ast.UInt | UReal -> bodyfn var
  | UVector | URowVector | UMatrix -> mkfor ut bodyfn var
  | UArray t -> mkfor ut (fun e -> for_scalar t bodyfn e) var
...
let rec for_eigen ut bodyfn var =
  match ut with
  | Ast.UInt | UReal | UVector | URowVector | UMatrix -> bodyfn var
  | UArray t -> mkfor ut (fun e -> for_eigen t bodyfn e) var
  | UFun _ | UMathLibraryFunction ->
      raise_s [%message "Can't for over " (ut : unsizedtype)]
